...
class Huggable extends Trait {
    static override readonly is: (input: unknown) => input is Huggable =
        isShape({
            hug: isFunc
        })
...
describe('instanceof uses type guards', () => {
    ...
    it("doesn't need to be an instance", () => {
        const huggable = {
            hug() {
                //
            }
...
test('throws if static is has not been implemented', () => {
    expect(() => BrokenTrait.is(teddyBear)).toThrow(
        "BrokenTrait has not implemented a static type guard named 'is'"
    )
})

test('classes created by calling traits.use have a composed type guard', () => {
